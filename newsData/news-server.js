const newsContainer = document.querySelector(".posts-container")
let finalResult;



async function fetchingNews() {

    const dayDate = new Date().getDate();
    console.log()



    const news = await fetch(`https://newsapi.org/v2/everything?q=ferrari&from=2024-05-0${dayDate}&sortBy=publishedAt&apiKey=debd6da737ae44cbaad25e59d7eb30e6`);
    const newsData = await news.json();
    const articles = newsData.articles


    let articlesList = [];
    for(article in articles) {
        articlesList.push(articles[article]);

    }

    console.log(articlesList
    )

    finalResult = `
         <div class="post">
                    <img src="${articlesList[0].urlToImage}" alt="">
                <div class="post-content">
                    <div class="desc">
                        <h3>${articlesList[0].title}</h3>
                        <p> ${articlesList[0].description}</p>
                    </div>            
                    <div class="info">
                        <div class="post-info">
                            <span>
                                <img src="data:image/jpeg;base64,"  alt="">
                            </span>
                            <div class="info-txt">
                                <span class="author">
                                    ${articlesList[0].author == null || articlesList[0].author.length > 25 ? "unknown" : articlesList[0].author}
                                </span>
                                <span class="date">
                                    11/03/2024
                                </span>
                            </div>
                        </div>
                        <a href="${articlesList[0].url}">Read More</a>
                    </div>
                </div>
                </div>
    

                <div class="post">
                    <img src="${articlesList[1].urlToImage}" alt="">
                <div class="post-content">
                    <div class="desc">
                        <h3>${articlesList[1].title}</h3>
                        <p> ${articlesList[1].description}</p>
                    </div>            
                    <div class="info">
                        <div class="post-info">
                            <span>
                                <img src="data:image/jpeg;base64,"  alt="">
                            </span>
                            <div class="info-txt">
                                <span class="author">
                                ${articlesList[1].author == null || articlesList[1].author.length > 25 ? "unknown" : articlesList[1].author}
                                </span>
                                <span class="date">
                                    11/03/2024
                                </span>
                            </div>
                        </div>
                        <a href="${articlesList[1].url}">Read More</a>
                    </div>
                </div>
                </div>

                <div class="post">
                    <img src="${articlesList[2].urlToImage}" alt="">
                <div class="post-content">
                    <div class="desc">
                        <h3>${articlesList[2].title}</h3>
                        <p> ${articlesList[2].description}</p>
                    </div>            
                    <div class="info">
                        <div class="post-info">
                            <span>
                                <img src="data:image/jpeg;base64,"  alt="">
                            </span>
                            <div class="info-txt">
                                <span class="author">
                                ${articlesList[2].author == null || articlesList[2].author.length > 25 ? "unknown" : articlesList[2].author}
                                </span>
                                <span class="date">
                                    11/03/2024
                                </span>
                            </div>
                        </div>
                        <a href="${articlesList[2].url}">Read More</a>
                    </div>
                </div>
                </div>
    
    
    `;


    newsContainer.innerHTML = finalResult;
    
}

fetchingNews()